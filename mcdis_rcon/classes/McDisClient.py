...
ProcessGroupConfig = dict[str, ProcessConfig]


class ProcessesTypesConfig(TypedDict):
    Networks: ProcessGroupConfig
    Servers: ProcessGroupConfig
...
McDisClientConfig = TypedDict(
    'McDisClientConfig',
    {
        ...
        'Bot Prefix': NotRequired[str | None],
    },
)
...
class McDisClient(commands.Bot):
    def __init__(self) -> None:
        ...
        self.prefix: str = '!!'
        self.cwd = os.getcwd()
        self.config: McDisClientConfig = cast(McDisClientConfig, {})
        self.panel = cast(discord.TextChannel, None)
        self._ = cast(Callable[[str], str], None)
    ...
    def _load_panel(self) -> None:
        ...
        panel = self.get_channel(panel_id)
    ...
    async def _load_addons(self, *, reload: bool = False) -> None:
        if reload:
            self.unload_addons()
        ...
        addons = [
            file
            for file in files_in_addons_dir
    ...
    async def _load_banner(self, *, loop: bool = True, view: bool = True) -> None:
        from ..gui.Panel import PanelEmbed, PanelView
        ...
        while True:
            try:
                ...
                try:
                    self.panel = cast(discord.TextChannel, await self.fetch_channel(self.panel.id))
                except Exception:
    ...
    def console_listener(self) -> None:
        try:
            while True:
                print(
    ...
    async def panel_interface(self, message: discord.Message) -> None:
        ...
        if message.channel.id == self.panel.id:
            if self.is_command(message.content.lower(), 'start-all'):
                ...
                response = await message.channel.send(self._('✔ Initializing processes.'))
                ...
                response = await message.channel.send(self._('✔ Stopping processes.'))
                await response.delete(delay=2)
                ...
                response = await message.channel.send(self._('✔ Forcibly stopped processes.'))
                await response.delete(delay=2)
                ...
                response = await message.channel.send(self._('✔ Restarting processes...'))
                await response.delete(delay=2)
                ...
                response = await message.channel.send(self._('✔ Reloading mdplugins...'))
                await response.delete(delay=2)
                ...
                response = await message.channel.send(self._('✔ Reloading mdaddons...'))
                await response.delete(delay=2)
                await self._load_addons(reload=True)
    ...
    async def restart(self, interaction: discord.Interaction[Any]) -> None:
        self.unload_addons()
        if interaction.message is None:
            return

        await interaction.response.edit_message(
            content=self._('Checking if there are open processes...'), embed=None, view=None
        )

        await asyncio.sleep(1)

        def any_process_open() -> bool:
            return any(process.is_running() for process in self.processes)

        if any_process_open():
            await interaction.followup.edit_message(
                message_id=interaction.message.id, content=self._('Closing processes...')
            )
    ...
    async def error_report(self, *, title: str, error: str, should_print: bool = True) -> str:
    ...
    def error_wrapper(
        self, *, error_title: str = '', reports: dict[str, bool] | None = None
            ...
            def wrapped(*args: Any, **kwargs: Any) -> Any:
                try:
                    ...
                    asyncio.run_coroutine_threadsafe(
                        self.error_report(title=error_title, error=traceback.format_exc()),
                        self.loop,
                    )
